# Dataverse Table Definitions for Power Pages

These are the same Dataverse tables used by the Power Apps Canvas export.
If you already created them for Power Apps, skip ahead to **Site_Configuration.md**.
...
| Value Score | `cr7b4_ValueScore` | Decimal (2 places) | No | Calculated |
| Feasibility Score | `cr7b4_FeasibilityScore` | Decimal (2 places) | No | Calculated |
| Quadrant | `cr7b4_Quadrant` | Choice | No | Calculated |
...
These formulas are implemented in JavaScript (`app.js`) rather than Dataverse calculated
columns, because Power Pages Web API allows client-side computation before saving.

### Value Score (1.0 – 3.0)

```
ValueScore = (BusinessGrowth × 0.20)
           + (CostEfficiency × 0.20)
           + (BusinessResilience × 0.30)
           + (BusinessAgility × 0.30)
```

### Feasibility Score (1.0 – 3.0)

```
FeasibilityScore = (TechnicalFeasibility × 0.50)
                 + (InternalReadiness × 0.30)
                 + (ExternalReadiness × 0.20)
```
